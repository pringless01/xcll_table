(function waitForCore() {
  ...
  const { NS, state, ensureRoot, showToast, throttle } = window.HKYY;
  ...
  function ensurePasteBubble() {
    ...
    el.querySelector(`#${NS}-btn-paste`).addEventListener("click", () => onPaste(false));
    el.querySelector(`#${NS}-btn-clearpaste`).addEventListener("click", () => onPaste(true));
    return el;
  }
  ...
  function positionFor(el) {
    ...
    let top = window.scrollY + r.top - bh - 6; // dış sol-üst
    let left = window.scrollX + r.left;
    if (top < window.scrollY) top = window.scrollY + r.top + 2; // üst yoksa içe al
  ...
  async function onPaste(clearBefore) {
    ...
    if (!target || !(target instanceof Element)) { hide(ensurePasteBubble()); return; }
  ...
  document.addEventListener("focusin", (e) => {
    ...
    } else { hide(ensurePasteBubble()); state.currentPasteTarget = null; }
  }, { passive: true });

  const rePos = throttle(() => { if (state.currentPasteTarget) positionFor(state.currentPasteTarget); }, 50);
  ...
  document.addEventListener("click", (e) => {
    ...
    if (!insideUI && !clickedTarget && Date.now() >= state.pasteBubbleStickyUntil) hide(ensurePasteBubble());
  }, { capture: true });
})();
