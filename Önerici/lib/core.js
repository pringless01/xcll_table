(function () {
  ...
  const state = {
    ...
    highlights: [],
    MAX_HIGHLIGHTS: 200,
    els: { root: null, copyBubble: null, pasteBubble: null, searchBar: null, toast: null, btn: {} },
    toolbarPos: { top: 8, left: null, right: null }, // persisted
  };
  ...
  function showToast(text, ms = 1400) {
    ...
    showToast._t = setTimeout(() => (state.els.toast.style.display = "none"), ms);
  }
  ...
  function throttle(fn, wait=50){
    ...
    return function(...args){
      const now=Date.now();
      pending=args;
      const run=()=>{ last=now; tid=null; fn.apply(this,pending); pending=null; };
      if(!last || (now-last)>=wait){ run(); }
      else if(!tid){ tid=setTimeout(run, wait-(now-last)); }
    };
  }
  ...
  function cacheLastCopied(text) {
    ...
    try { chrome.storage.local.set({ lastCopied: text }); } catch {}
  }

  async function refreshClipboardCache() {
    try {
      const txt = await navigator.clipboard.readText();
      if (txt && typeof txt === "string" && txt !== state.lastCopied) cacheLastCopied(txt);
    } catch {}
  }
  ...
  // toolbar position persist + realtime
  ...
  function applyToolbarPos(pos){
    ...
    if (!el) return;
    // varsayılan merkezlenmiş tasarım yerine kesin konumlanma
    ...
    el.style.transform = ""; // artık translateX(-50%) yok
  }

  async function loadInitialState(){
    return new Promise((res)=> {
      chrome.storage.local.get(["lastCopied", "hkyy_toolbar_pos"], (out) => {
        if (typeof out.lastCopied === "string") state.lastCopied = out.lastCopied;
  ...
  window.HKYY = {
    NS, state, ensureRoot, showToast, escapeRegExp, cacheLastCopied,
    refreshClipboardCache, clearHighlights, smoothCenter, throttle,
    saveToolbarPos, applyToolbarPos, loadInitialState
  };
})();
