if (window.__EXCEL_HELPER_MODULAR__) {
  console.log('Legacy content.js skipped (modular active)');
  return;
}
...
function clearSelection() {
  ...
  const selectedElements = document.querySelectorAll(
    '.selected-cell, .selected-row, .selected-col'
  );
...
function updateStatusInner() {
  ...
  const [visibleSelectedCells, visibleSelectedRows, visibleSelectedCols] =
    visibleCounts;
  const totalVisible =
    visibleSelectedCells + visibleSelectedRows + visibleSelectedCols;
  const totalSelected =
    allSelectedCells.length + allSelectedRows.length + allSelectedCols.length;
...
function setupAdvancedSelection() {
  ...
  const eventHandlers = {
    globalMouseDown: function (e) {
      ...
      if (
        !clickedElement.closest('#excel-helper-toolbar') &&
        ...
        // Use e.ctrlKey instead of global ctrlPressed
        clearSelection();
        updateStatus();
      }
    },
  ...
  document.addEventListener('mousedown', eventHandlers.globalMouseDown, {
    passive: true,
  });
  document.addEventListener('mouseup', eventHandlers.globalMouseUp, {
    passive: true,
  });
  ...
  tables.forEach((table) => {
    ...
    table.addEventListener('mouseenter', tableHandlers.mouseenter, {
      capture: true,
      passive: true,
    });
...
function addHeaderStyles(table) {
  ...
  for (let r = 1; r < table.rows.length; r++) {
    ...
    if (
      !row ||
      row.classList.contains('table-filter-row') ||
...
function updateTableTotalRow(table) {
  ...
  selectedCells.forEach((cell) => {
    if (
      cell.classList.contains('table-filter-row') ||
      cell.classList.contains('row-header') ||
      cell.classList.contains('col-header') ||
  ...
  if (message.type === 'copy-selection') {
    ...
    navigator.clipboard
      ...
      .catch(() => {
        sendResponse({
          success: false,
          message: 'Clipboard erişimi başarısız!',
        });
      });
...
async function init() {
  ...
  function scheduleNextTask() {
    if (scheduleTasks.length === 0) {
      ...
      console.log(
        `✅ Excel Helper başlatıldı - Performance Optimized (${initTime.toFixed(2)}ms)`
      );
      console.log(
        `📍 Toolbar position: ${userSettings.toolbarPosition.x}, ${userSettings.toolbarPosition.y}`
      );
      console.log(
        `🎯 Selection mode: ${userSettings.selectionMode ? 'ENABLED' : 'DISABLED'}`
      );
      return;
    }
