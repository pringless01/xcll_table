...
class ExcelHelperTestManager {
  ...
  createBackup(type = 'manual') {
    try {
      ...
      console.error('Backup oluşturulamadı');
    }
  }
  ...
  testCalculationSystem() {
    ...
    try {
      ...
      const isValid = results.every(
        (result, index) => Math.abs(result - expected[index]) < 0.01
      );
  ...
  sendToGitHub(data) {
    ...
    if (webhookURL) {
      ...
      }).catch(() => console.warn('GitHub sync failed'));
    }
  }
  ...
  getStatus() {
    return {
      ...
      lastBackup: this.getFromStorage('excel_helper_backups')?.slice(-1)[0]
        ?.timestamp,
      performance: this.getPerformanceMetrics(),
    };
  }
