...
(function(){
  function createBus(){
    ...
    return {
      ...
      emit(evt, payload){ const set = map.get(evt); if(set){ set.forEach(fn=>{ try{ fn(payload);}catch(e){ console.error('[ExcelHelper][EventBus] handler error', evt, e);} }); } }
    };
  }
  window.ExcelHelperNS = window.ExcelHelperNS || {};
  if(!window.ExcelHelperNS.eventBus) window.ExcelHelperNS.eventBus = createBus();
})();
