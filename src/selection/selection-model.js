...
(function () {
  if (window.__EH_SELECTION_MODEL_LOADED) return;
  window.__EH_SELECTION_MODEL_LOADED = true;
  ...
  function clearSelection() {
    ...
    state.cells.forEach((c) => {
      c.classList.remove(CLS, 'selected-cell', 'selected-row', 'selected-col');
      c._ehSel = false;
    });
    state.cells.clear();
    state.version++;
  ...
  function addColumn(table, colIndex) {
    if (!table) return;
    let added = false;
  ...
  function snapshot() {
    if (lastSnapshot && lastSnapshotVersion === state.version)
      return lastSnapshot;
    const cells = getSelectedCells();
    const numericValues = [];
    cells.forEach((c) => {
      if (c._ehNum !== undefined) {
        if (c._ehNum !== null) numericValues.push(c._ehNum);
        return;
      }
      const v = window.ExcelHelperNS.parseNumericValue(c.textContent.trim());
      c._ehNum = v;
      if (v !== null) numericValues.push(v);
    });
    lastSnapshot = { cells, numericValues, version: state.version };
  ...
  let sweepTimer = setInterval(sweepOrphans, 5000);
  function stopSweep() {
    if (sweepTimer) {
  ...
  function startSweep() {
    if (!sweepTimer) {
      sweepTimer = setInterval(sweepOrphans, 5000);
    }
  }
